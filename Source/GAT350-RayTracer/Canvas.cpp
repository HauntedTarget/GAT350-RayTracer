...
namespace nc
{
	Canvas::Canvas(int width, int height, const Renderer& renderer)
	{
		...
		m_buffer.resize(static_cast<std::vector<nc::Color::rgba_t, std::allocator<nc::Color::rgba_t>>::size_type>(width) * height); //<resize vector to hold width* height elements>
	}
	...
	void Canvas::Update()
	{
		...
		SDL_UpdateTexture(m_texture, NULL, m_buffer.data(), m_size.x * sizeof(Color::rgba_t));
	}
	...
	void Canvas::DrawPoint(const glm::ivec2& point, const Color::color4_t& color)
	{
		...
		m_buffer[point.x + (static_cast<std::vector<nc::Color::rgba_t, std::allocator<nc::Color::rgba_t>>::size_type>(point.y) * m_size.x)] = Color::ColorToRGBA(color);
	}
}